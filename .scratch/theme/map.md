...
- [Scaffold the repo and emit the first port](issues/06-scaffold-the-repo.md) — **built and committed locally** (`palette.json`, typed `src/palette.ts`, `Emitter` seam, `bun run build` / `bun run check`, Ghostty port byte-identical to the design, 43 tests green). Remote pending: jass runs `gh repo create jassuwu/lichen --public --source . --remote origin --push`.
