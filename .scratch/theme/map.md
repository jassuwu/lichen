...
- [Syntax legibility: telling functions, strings and keywords apart](issues/14-syntax-legibility.md) — **reopened on jass's review of the palette ("can't tell a function from a string; the lime is underutilized"). Answer: keywords dim to `subtle`, function names take the accent (its one job in code), types on a returning `bright` step, strings boxed on an `overlay` highlight rather than coloured, punctuation `muted`.** Amends the vocabulary and the palette; eight greys now. Prototype at `.scratch/theme/prototype/syntax.html?variant=E`. **Second round 2026-09-02 (jass: "not very utilitarian"): the greys stay, the other channels open — grammar italic, definitions bold, parameters italic, literals `bright` (strings too, boxed where possible; vs code draws no token backgrounds, which was silently flattening strings there). `palette.json` `syntax` entries carry `bold`/`italic`/`background`; nvim and vs code both read it.** Prototypes at `.scratch/theme/proto/`.
...
- [Scaffold the repo and emit the first port](issues/06-scaffold-the-repo.md) — **built and committed locally** (`palette.json`, typed `src/palette.ts`, `Emitter` seam, `bun run build` / `bun run check`, Ghostty port byte-identical to the design, 43 tests green). Remote: [github.com/jassuwu/lichen](https://github.com/jassuwu/lichen).
- [Ghostty port, installed](issues/07-ghostty-port.md) — **installed**, `theme = lichen`.
- [Neovim port, installed](issues/08-neovim-port.md) — **installed** via `vim.pack` from the GitHub repo; ~330 generated groups incl. treesitter, LSP semantic, gitsigns, fzf-lua, oil, mini; `setup({ transparent })` is the only option.
- [VS Code / Cursor theme, published to Marketplace and Open VSX, verified in t3code](issues/09-vscode-port-published.md) — **built, packaged, installed in Cursor; publishing needs jass's accounts** — checklist in the ticket (Marketplace publisher + PAT, Open VSX namespace + token, then verify in t3code).
- [herdr matches](issues/10-herdr-port.md) — **installed**: `name = "terminal"` + ui overrides, reloaded.
- [p10k prompt matches](issues/11-p10k-colours.md) — **installed**: prompt char is the accent, dirs are text, git is muted.
- [The wallpaper](issues/12-wallpaper.md) — **done, then grown into a set of sixteen flavors (2026-08-31)**: the five quiet ones (`rock`, `spore`, `grid`, `rings`, `wordmark`) plus eleven from the generative-art canon on jass's "go crazy" review (`flow`, `ridge`, `dither`, `attractor`, `maze`, `phyllo`, `moire`, `turing`, `branch`, `tty`, `bands`), all duotone grey + lime, × two display sizes, rendered by `Bun.WebView`, rock set on every desktop.
- **README with branding (2026-09-02)** — `src/emit/readme.ts` renders `docs/assets/{banner,palette,code}.png` and a thumbnail per wallpaper (`Bun.Image` downscale of the committed file) as build outputs, `check: false` like the wallpapers. README rewritten around them: pitch, syntax sample, swatches + role table, per-port install, wallpaper gallery, build. A landing page is proposed, not built — see the discoverability note below.

## Not yet specified

- **How the theme reaches the other laptop.** This machine's ghostty config, nvim `plugins.lua` and `.p10k.zsh` are dotfiles-tracked and now modified but uncommitted there; herdr's config and Cursor's settings are not in dotfiles. Commit the dotfiles changes, reconcile the other laptop's unpushed kanso overrides, install the `.vsix` there (or wait for the marketplace).
- **Discoverability beyond "installed on jass's machine".** (2026-09-02: jass asked whether a landing site is worth it; recommendation given — a single static page on a jass.gg subdomain built from the same emitters, once the repo is public and the VS Code port is sideloadable; the README carries it until then.) A PR to Ghostty's bundled themes (via iTerm2-Color-Schemes), a listing in nvim colorscheme indexes, a small page for the theme (on jass.gg or its own subdomain?). Which of these are wanted, and whether any belong to *this* map or a follow-up, waits on the theme existing.
